...
Question: when C1 trains WELL at a higher learning rate, does its gate go bimodal?

We sweep C1 across learning rates between the healthy 1e-3 and the broken 3e-3.
For each: read the training loss and the gate open %.


- Keep only runs that reach ~3.3 train loss (those converged).
- Among the converged runs, look at open %.
    open ~0%   -> C1 stays a suppressor even at the highest stable LR.
    open high  -> C1 goes bimodal WHILE still training well (would weaken the claim).
- If no LR above 1e-3 converges, C1 simply has no stable high-LR regime.

C3 @ 3e-3 is included as the stable reference.

Each line: train_loss (best) | open% | closed% | mean gate | mean|logit|
"""
...
def run(dataset, vocab, qk, pre, post, lr, seed=12, epochs=10, layers=8):
    ...
    for ep in range(epochs):
        ...
        for xb, yb in tl:
            ...
            if not torch.isfinite(loss):
                ...
                return {"train": float("nan"), "open": float("nan"),
                        "closed": float("nan"), "mean": float("nan"), "logit": float("nan")}
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
        best_train = min(best_train, loss.item())
    ...
    with torch.no_grad():
        for i, (xb, yb) in enumerate(vl):
            ...
            n  += 1
    return {"train": best_train, "open": 100 * o / n, "closed": 100 * c / n,
            "mean": m / n, "logit": lg / n}
...
if __name__ == "__main__":
    ...
    # (name, qk_norm, pre_norm, post_norm, lr)  -- C1 swept between healthy and broken
    RUNS = [
        ("C1 @ 1.0e-3  (known good)",       False, True, False, 1.0e-3),
        ("C1 @ 1.5e-3  (higher, stable?)",  False, True, False, 1.5e-3),
        ("C1 @ 2.0e-3  (the decider)",      False, True, False, 2.0e-3),
        ("C1 @ 2.5e-3  (near the edge)",    False, True, False, 2.5e-3),
        ("C1 @ 3.0e-3  (known broken)",     False, True, False, 3.0e-3),
        ("C3 @ 3.0e-3  (stable reference)", True,  True, True,  3.0e-3),
    ]
    ...
    for name, qk, pre, post, lr in RUNS:
        ...
        out.append((name, r))
        print(f"  -> train_loss={r['train']:.2f}  open={r['open']:.2f}%  "
              f"closed={r['closed']:.2f}%  mean={r['mean']:.3f}  |logit|={r['logit']:.2f}\n", flush=True)

    print("\n-------------------- SUMMARY --------------------")
    print(f"{'run':<36}{'train_loss':>11}{'open':>8}{'closed':>9}{'|logit|':>9}  converged?")
    for name, r in out:
        conv = "yes" if r["train"] <= 3.45 else "NO (broke)"
        print(f"{name:<36}{r['train']:>11.2f}{r['open']:>7.2f}%{r['closed']:>8.2f}%{r['logit']:>9.2f}  {conv}")

    print("\n-------------------- READING --------------------")
    print("Among C1 runs that CONVERGED (train_loss <= ~3.45):")
    print("  - if open% stays near 0  -> C1 is a suppressor even at the highest stable LR.")
    print("  - if open% goes high     -> C1 bimodal while training well (weakens the claim).")
    print("The highest-LR C1 that still converged is the one that settles it.")
